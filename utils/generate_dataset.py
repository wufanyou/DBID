# Modify from https://github.com/ulaval-damas/tree-bark-classification/blob/master/src/dataset/generate_dataset.py
import json
import os
import math
...
class GenerateDataset:
    ...
    def get_k_fold_format_dataset(self, fold_num, k,):
        t = self.get_k_fold_dataset(k)[fold_num]
        train = [(t['train']['files'][i],t['train']['labels'][i][0]) for i in range(len(t['train']['files']))]
        test = [(t['test']['files'][i],t['test']['labels'][i][0]) for i in range(len(t['test']['files']))]
        return train,test
